<div id="carouselCaptions" class="carousel slide mb-4" data-bs-ride="carousel">
        ...
        <?php foreach ($articlesCarousel as $key => $article) : ?>
            <a href="/article?id=<?= $article->id ?>">
                <div class="carousel-item <?= ($key === 0) ? 'active' : '' ?>" data-bs-interval="4000">
                    <img src="/img/<?= $detect->isMobile() ? 'mobile_resolution' : 'uploads' ?>/<?= $article->formFile ?>" class="d-block w-100" alt="<?= pathinfo($article->formFile, PATHINFO_FILENAME) ?>" width="1280">
                    <span class="position-absolute top-0 end-0 mt-3 me-3">
                        <button class="btn btn-secondary opacity-75 p-0">
                            <span class="badge text-bg-secondary">
                                <em><?= $db->getCategoryById($article->category_id)->name ?></em>
                            </span>
                        </button>
                    </span>
